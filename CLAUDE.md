...
- **Content:** All copy lives in `src/data/` — `nav.js`, `about.js`, `aboutMe.js`, `projects.js`, `experience.js`, `education.js`, `agent.js`. Never hardcode content inside components. `aboutMe.js` exports `ABOUT_ME_STATEMENT`, `ABOUT_ME_HIGHLIGHT`, `ABOUT_ME_EMPHASES` used by `AboutMe.jsx`.
...
- **Section render order in `App.jsx`:** `Nav → Hero → AboutMe → About → Experience → Education → Projects → Footer`. `AIOrb` + `AIDrawer` + `Cursor` are fixed overlays at the end of the tree.
